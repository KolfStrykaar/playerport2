...
void plague_effect(void *vo,int level, int dam, int target)
{
    if (target == TARGET_ROOM)  /* nail objects on the floor */
    {
        ...
        {
            obj_next = obj->next_content;
            plague_effect(obj,level,dam,TARGET_OBJ);
        }
        return;
    }
	...
	/* chance of plaguing */
        if (!saves_spell(level / 4 + dam / 20,victim,DAM_ENERGY))
        {
	    AFFECT_DATA af;

            send_to_char("You feel a plague coursing through your body.\n\r",
                victim);
            act("$n looks like the walking dead.",victim,NULL,NULL,TO_ROOM);
