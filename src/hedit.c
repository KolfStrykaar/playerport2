...
HEDIT(hedit_level)
{
	...
	{
		printf_to_char( ch, "HEdit : level should be between -1 and %d .\n\r", MAX_LEVEL );
		return FALSE;
	}
...
void hedit( CHAR_DATA *ch, char *argument)
{
    ...
    if (had == NULL)
    {
    	bugf( "hedit : No area to add the help %s", pHelp->keyword );
    	edit_done(ch);
    	return;
    }
    ...
    {
        send_to_char("HEdit: You dont have the security to edit helps.\n\r",ch);
	edit_done(ch);
	return;
    }
...
HEDIT(hedit_delete)
{
	...
	else
	{
		...
		if ( !temp )
		{
			bugf( "hedit_delete : help %s not found", pHelp->keyword );
			return FALSE;
		}
