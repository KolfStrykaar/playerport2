...
void load_mobiles( FILE *fp )
{
	...
	pMobIndex->material		= str_dup(fread_word( fp ));
 	pMobIndex->org_id			= fread_number(fp);
...
void load_objects( FILE *fp )
{
	...
	CHECK_POS(pObjIndex->item_type, item_lookup(fread_word( fp )), "item_type" );
        ...
        /*Give the object it's size*/
        pObjIndex->size                 = SIZE_MEDIUM;

        /*change size depending on wear flag*/
        if(IS_SIZE(pObjIndex, ITEM_SIZE_SMALL))
          pObjIndex->size = SIZE_SMALL;
        else if(IS_SIZE(pObjIndex, ITEM_SIZE_TINY))
          pObjIndex->size = SIZE_TINY;
        else if(IS_SIZE(pObjIndex, ITEM_SIZE_MEDIUM))
          pObjIndex->size = SIZE_MEDIUM;
        else if(IS_SIZE(pObjIndex, ITEM_SIZE_LARGE))
          pObjIndex->size = SIZE_LARGE;
        else if(IS_SIZE(pObjIndex, ITEM_SIZE_GIANT))
          pObjIndex->size = SIZE_GIANT;
        else if(IS_SIZE(pObjIndex, ITEM_SIZE_HUGE))
          pObjIndex->size = SIZE_HUGE;
        else if(IS_SIZE(pObjIndex, ITEM_SIZE_IGNORE))
          pObjIndex->size = SIZE_IGNORE;
	...
	case ITEM_BOOBYTRAP:
	case ITEM_TRAP:
	    CHECK_POS(pObjIndex->value[1], trap_lookup(fread_word(fp)), "trap_lookup" );
            break;
	default:
...
void convert_object( OBJ_INDEX_DATA *pObjIndex )
{
    ...
    pObjIndex->cost     = 10*level;
        /*change size depending on wear flag*/
        if(IS_SET(pObjIndex->wear_flags, ITEM_SIZE_TINY))
          pObjIndex->size = SIZE_TINY;
        else if(IS_SET(pObjIndex->wear_flags, ITEM_SIZE_SMALL))
          pObjIndex->size = SIZE_SMALL;
        else if(IS_SET(pObjIndex->wear_flags, ITEM_SIZE_MEDIUM))
          pObjIndex->size = SIZE_MEDIUM;
        else if(IS_SET(pObjIndex->wear_flags, ITEM_SIZE_LARGE))
          pObjIndex->size = SIZE_LARGE;
        else if(IS_SET(pObjIndex->wear_flags, ITEM_SIZE_GIANT))
          pObjIndex->size = SIZE_GIANT;
        else if(IS_SET(pObjIndex->wear_flags, ITEM_SIZE_HUGE))
          pObjIndex->size = SIZE_HUGE;
        else if(IS_SET(pObjIndex->wear_flags, ITEM_SIZE_IGNORE))
          pObjIndex->size = SIZE_IGNORE;
        else
	    pObjIndex->size             = SIZE_MEDIUM;
...
char *center( char *argument, int width, char *fill)
{
    ...
    if( !argument )
    {
        bugf("Problem in center command");
        return log_buf;
    }
