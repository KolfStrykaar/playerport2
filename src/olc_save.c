...
void save_mobile( FILE *fp, MOB_INDEX_DATA *pMobIndex )
{
    ...
    {
    	fprintf( fp, "%s ", pMobIndex->material );
    }
 ...
 */
	...
	case ITEM_ARROW:
        ...
        case ITEM_TRAP:
            fprintf( fp, "'%s'\n",
                     trap_table[pObjIndex->value[1]].trap_name);
            break;
    }
...
void save_rooms( FILE *fp, AREA_DATA *pArea )
{
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
		...
		fprintf( fp, "0 " );
			...
			||   IS_SET(pExit->rs_flags, EX_MAGICSEAL) )
...
/* Builders may only use ASAVE AREA. All other asave functions are IMP/MAX SECURITY ONLY
 * asave list, asave changed, asave helps, asave world are RESTRICTED to above credentials. */
	if( ch != NULL
	&&(!IS_IMPLEMENTOR(ch) && ch->pcdata->security < 9)
	&& !is_number(arg1)
	&&(!str_cmp(arg1, "list")
	|| !str_cmp(arg1, "changed")
	|| !str_cmp(arg1, "helps")
	|| !str_cmp(arg1, "world")) )
	{
		send_to_char("You are not authorized to use this command.\n\r"
			"To save your area, type the following three commands: AEDIT, ASAVE AREA, DONE\n\r",ch);
		return;
	}
    ...
    if ( pArea->helps && pArea->helps->first )
	save_helps( fp, pArea->helps );
	send_to_char("All help files saved.\n\r",ch);
	return;
    }
