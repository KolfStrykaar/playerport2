...
void string_add( CHAR_DATA *ch, char *argument )
{
    ...
    {
      if (IS_SET(ch->comm,COMM_WRITING))
	REMOVE_BIT(ch->comm,COMM_WRITING);

      if (IS_SET(ch->comm,COMM_EDITOR))
	REMOVE_BIT(ch->comm,COMM_EDITOR);
...
char *mygetline( char *str, char *buf )
{
	int tmp = 0;
	bool found = FALSE;
