...
void do_hunt( CHAR_DATA *ch, char *argument )
{
  ...
  if( IS_AFFECTED2(victim, AFF_PASSTRACE ))
    {
 	send_to_char ( "You see no trace of their passing whatsoever.\n\r", ch);
	return;
    }
...
void hunt_victim( CHAR_DATA *ch )
{
  ...
  if (ch->hunting->hit < ch->hunting->max_hit * .2)
  {
    ch->hunting = NULL;
    return;
  }
  ...
  {
    if( IS_AFFECTED2(ch->hunting, AFF_INVUL) 
    ||  ch->hunting->position == POS_DEAD
    ||  IS_AFFECTED3(ch->hunting, AFF_SUBDUE))
    {
      ch->hunting = NULL;
      return;
    }
...
void do_track( CHAR_DATA *ch, char *argument )
{
  ...
  if( !IS_NPC(ch) && get_skill(ch, gsn_track) < 1 )
  {
	send_to_char("Huh?\n\r",ch);
	return;
  }
