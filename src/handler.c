...
int get_align_change ( int align1, int align2)
{
    float incr;
    int i1, i2;
    int aldiff1, aldiff2;
    
    if ( align1 > ALIGN_GOOD_LOW )
    {    
        i1= 0;
        aldiff1 = ALIGN_GOOD_HIGH - align1;
    }
    else if ( align1 < ALIGN_EVIL_LOW )
    {
        i1 = 2;
        aldiff1 = 0 - (ALIGN_EVIL_HIGH - align1);
    }
    else 
    {
        i1 = 1;
        aldiff1 = ALIGN_GOOD_LOW - align1;
    } 


    if ( align2 > ALIGN_GOOD_LOW )
    {    
        i2= 0;
        aldiff2 = ALIGN_GOOD_HIGH - align2;
    }
    else if ( align2 < ALIGN_EVIL_LOW )
    {
        i2 = 2;
        aldiff2 = 0 - (ALIGN_EVIL_HIGH - align2);
    }
    else 
    {
        i2 = 1;
        aldiff2 = ALIGN_GOOD_LOW - align2;
    } 

    
    incr = ABS(( ( align_change_table[i1][i2].high -
               align_change_table[i1][i2].low ) / 666 ));
    
   return ( align_change_table[i1][i2].high - 
                     (((aldiff1+aldiff2)/2) * incr ));              
...
/* returns religion number */
int religion_lookup (const char *name)
{
   int religion;
   for ( religion = 0; (religion_table[religion].longname != NULL)
   && (religion != MAX_RELIGION); religion++)
   {
     if (LOWER(name[0]) == LOWER(religion_table[religion].longname[0])
     &&  !str_prefix( name,religion_table[religion].longname))
       return religion;
   }
   return 0;
} 
...
int material_lookup (const char *name)
{
   ...
   for ( material = 0; material_list[material].name != NULL; material++)
   {
	if (LOWER(name[0]) == LOWER(material_list[material].name[0])
	&&  !str_prefix( name,material_list[material].name))
	    return material;
   }
...
int forge_lookup(const char *name)
{
	int item;
   	...
   	{
		if ( LOWER(name[0]) == LOWER(forge_items_table[item].name[0])
		&&  !str_prefix(name, forge_items_table[item].name) )
	    		return item;
   	}
...
int get_age( CHAR_DATA *ch )
{
   ...
   if (IS_SET(ch->comm, COMM_SLOWAGE) && (!IS_NPC(ch)))
    return pc_race_table[ch->race].base_age + ( ((ch->played + (int) (current_time -
	ch->logon)) / 144000) );
   else
    return pc_race_table[ch->race].base_age + ( ((ch->played + (int) (current_time -
	ch->logon)) / 72000) );
...
/*
 * Get Age word in Ascii.
 */
char *get_age_word( CHAR_DATA *ch) 
{
    if (IS_NPC(ch))
    return ("npc");
    if (( (float) get_age(ch) / pc_race_table[ch->race].max_age) <= AGE_YOUTHFUL) return
("young");
    else if ((get_age(ch) / pc_race_table[ch->race].max_age) <= AGE_MATURE) return ("mature");
    else if ((get_age(ch) / pc_race_table[ch->race].max_age) <= AGE_MIDDLEAGED) return ("middle-aged");
    else if ((get_age(ch) / pc_race_table[ch->race].max_age) <= AGE_OLD) return ("old");
    else if ((get_age(ch) / pc_race_table[ch->race].max_age) <= AGE_ELDERLY) return ("elderly");
    else return ("ancient");
}

/*
 * Get Charisma word in Ascii.
 */
char *get_charisma( CHAR_DATA *ch, CHAR_DATA *vict )
{
   int charismastat = 0;
   
   charismastat = ch->perm_stat[STAT_CHR] + ch->mod_stat[STAT_CHR];
    if (!IS_NPC(vict))
    {
	if (vict->pcdata->condition[COND_DRUNK] > 5)
	{
	  charismastat += 10;
	 if (charismastat > 30)
	  charismastat = 30;
    ...
    if (charismastat > 25 ) 
      if (ch->sex == 2) return("a divinely beautiful"); else return("a divinely handsome");
    else if ( charismastat >  23 ) 
      if (ch->sex == 2) return("a stunningly beautiful"); else return("a stunningly handsome");
    else if ( charismastat >  20 ) 
      if (ch->sex == 2) return("a gorgeous"); else return("a dashing");
    else if ( charismastat >  17 )
      if (ch->sex == 2) return("a beautiful"); else return("a handsome");
    else if ( charismastat >  15 ) 
      if (ch->sex == 2) return("a lovely"); else return("an attractive");
    else if ( charismastat >  14 ) 
      if (ch->sex == 2) return("a pretty"); else return("a comely");
    else if ( charismastat >  13) return( "an average looking");
    else if ( charismastat >  12) 
      if (ch->sex == 2) return("a plain"); else return("a coarse"); 
    else if ( charismastat >  11) return( "an unattractive" );
    else if ( charismastat >  9 ) return( "an ugly" );
    else if ( charismastat >  7 ) return( "a hideous" );
    else                          return( "a repulsive" );
}
...
{
   if       ( ch->ethos == ETHOS_NEUTRAL ) return( "neutral" );
   else if  ( ch->ethos == ETHOS_CHAOTIC ) return( "chaotic" );
   else if  ( ch->ethos == ETHOS_LAWFUL  ) return( "lawful"  );
   else                                    return( "unknown" );
}
...
int can_carry_n( CHAR_DATA *ch )
{
    ...
    if ( !IS_NPC(ch) && ch->class == class_lookup("blacksmith") )
...
int can_carry_w( CHAR_DATA *ch )
{
    ...
    if ( !IS_NPC(ch) && ch->class == class_lookup("blacksmith") )
	return str_app[get_curr_stat(ch,STAT_STR)].carry * 10 + ch->level * 35;
...
bool is_name2 ( CHAR_DATA *ch, char *str, char *namelist )
{
    char name[MAX_INPUT_LENGTH], part[MAX_INPUT_LENGTH];
    char *list, *string;
    bool exact;
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
	...
	case TO_AFFECTS:
      ...
      case TO_AFFECTS2:
          ...
          break;
      case TO_AFFECTS3:
          SET_BIT(ch->affected3_by, paf->bitvector);
          break;
	case TO_IMMUNE:
    ...
    else
    {
        ...
        case TO_AFFECTS2:
            ...
            break;
        case TO_AFFECTS3:
            REMOVE_BIT(ch->affected3_by, paf->bitvector);
            break;
        case TO_IMMUNE:
	...
	bugf( "Affect_modify: sn %d, skill %s", paf->type, skill_table[paf->type].name );
	return;
    ...
    case APPLY_PERCEPTION:							break;
    case APPLY_SPEECH_IMPAIR:							break;
    }
	    ...
	    switch (where)
	    {
		    ...
		    break;
              case TO_AFFECTS2:
                ...
                break;
              case TO_AFFECTS3:
                SET_BIT(ch->affected3_by,vector);
                break;
	        case TO_IMMUNE:
...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
	...
	ROOM_INDEX_DATA *room;
        ...
        sprintf(buf,"Char_to_room: pRoomIndex == NULL, ch->name == %s.",ch->name);
        bug( buf, 0 );
...
int apply_ac( OBJ_DATA *obj, int iWear, int type )
{
    ...
    case WEAR_ANKLE_L:		return 2 * obj->value[type];
    case WEAR_ANKLE_R:		return 2 * obj->value[type];
    ...
    case WEAR_MARK:		return     obj->value[type];
    }
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    if ( get_eq_char( ch, iWear ) != NULL )
    {
        ...
        sprintf( log_buf, "ch=%s obj_vnum=%d",
...
void extract_char( CHAR_DATA *ch, bool fPull )
{
    ...
    */
    ...
    if ( !fPull )
    {
        ...
        /* send char to clan temple (room[1]) */
      /*if ( is_clan(ch) &&
           (get_room_index(clan_table[ch->clan].room[1]) != NULL) )
        char_to_room(ch,get_room_index(clan_table[ch->clan].room[1]));
      else
	...
	if ( wch->reply == ch )
	    wch->reply = NULL;
      if ( wch->hunting == ch )      /* shut down any huntings */
          wch->hunting = NULL;
...
CHAR_DATA *get_char_room( CHAR_DATA *ch, ROOM_INDEX_DATA *room, char *argument )
{
	...
	if ( (ch && !can_see( ch, rch )) || !is_name( arg, rch->name ) )
      ...
      && !IS_SET(ch->act,PLR_HOLYLIGHT)) ? !is_name2(ch,arg,rch->doppel->name) :
         !is_name2(ch, arg,rch->name) )
          continue;
...
void deduct_cost(CHAR_DATA *ch, int cost)
{
    ...
    if (ch->steel < 0)
    {
	bug("deduct costs: gold %d < 0",ch->steel);
	ch->steel = 0;
    }
    if (ch->gold < 0)
    {
 ...
 * Create a 'money' obj.
 */
OBJ_DATA *create_money( int steel, int gold )
{
    char buf[MAX_STRING_LENGTH];
    OBJ_DATA *obj;

    if ( steel < 0 || gold < 0 || (steel == 0 && gold == 0) )
    {
	bug( "Create_money: zero or negative money.",UMIN(steel,gold));
	steel = UMAX(1,steel);
	gold = UMAX(1,gold);
    }
    ...
    else if (steel == 1 && gold == 0)
    {
	obj = create_object( get_obj_index( OBJ_VNUM_STEEL_ONE), 0 );
    }
    else if (gold == 0)
    {
        obj = create_object( get_obj_index( OBJ_VNUM_STEEL_SOME ), 0 );
        sprintf( buf, obj->short_descr, steel );
	...
	sprintf( buf, obj->short_descr, gold, steel );
	free_string( obj->short_descr );
	obj->short_descr	= str_dup( buf );
	obj->value[0]		= gold;
	obj->value[1]		= steel;
	obj->cost		= 100 * steel + gold;
	obj->weight		= 0;
    }
...
bool room_is_dark( ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    if (pRoomIndex->vnum == ROOM_VNUM_ALTAR )
	return FALSE;
...
/* True if room has water for creatures */
bool room_has_water( ROOM_INDEX_DATA *pRoomIndex )
{
   if (!(pRoomIndex))
    {
	bug("Room_has_water, handler.c, pRoomIndex == NULL",0);
	return FALSE;
    }

    if (pRoomIndex->sector_type == SECT_WATER_SWIM ||
...
/* RT changed so that WIZ_INVIS has levels */
    ...
    if (!IS_NPC(ch) && (!IS_SET(ch->parts, PART_EYE||PART_EYESTALKS)))
        return FALSE; 
    ...
    if ( ch->in_room == NULL )
    {
	bug("Handler.c, Can_see: ch->in_room == NULL",0);
	sprintf(log_buf, "%s caused it.", ch->name ? ch->name : "(NULL name)");
	bug(log_buf, 0);
	return FALSE;
    }
    ...
    &&  IS_AFFECTED(ch, AFF_DETECT_INVIS))
    {
      int mod_detect = MOD_NONE;
      int mod_detlvl = MOD_NONE;
      int mod_wraith = MOD_NONE;
	AFFECT_DATA *af;
	bool af_found;
	af_found = FALSE;
	  ...
	  af_found = TRUE;
	{
	  if (af_found)
	    ...
	    for (af = ch->affected; af != NULL; af = af->next)
	    {
	      if (af->bitvector == AFF_DETECT_INVIS)
        	mod_detlvl = af->level;
	    }
        }
        else
	...
	{
	  if (af_found)
	    mod_wraith = victim->level;
        else
        for (af = victim->affected; af != NULL; af = af->next)
	  {
	    if (af->bitvector == AFF_WRAITHFORM)
        	mod_wraith = af->level;
	  }
      }

      if ( mod_wraith > mod_detect )
          return FALSE;

      if ( mod_wraith > ( mod_detlvl + 5 ) )
          return FALSE;
	...
	{
	  if (af_found)
	    mod_invis = victim->level;
        else for (af = victim->affected; af != NULL; af = af->next)
	  {
	    if (af->bitvector == AFF_INVISIBLE)
          mod_invis = af->level;
	  }
      }

        if ( mod_invis > mod_detect )
            return FALSE;

        if ( mod_invis > ( mod_detlvl + 5 ) )
          return FALSE;
    }
   ...
   &&  IS_AFFECTED(ch, AFF_DETECT_HIDDEN))
   {
	 ...
	 if (af_found)
	   ...
	   mod_detect = ch->level;
         for (af = ch->affected; af != NULL; af = af->next)
	   {
	     if (af->bitvector == AFF_DETECT_HIDDEN)
           mod_detlvl = af->level;
	   }
       }
       else
     ...
     if (IS_AFFECTED(victim, AFF_HIDE))
	   ...
	   {
	   if (af_found)
	   mod_hide   = victim->level;
         else for (af = victim->affected; af != NULL; af = af->next)
			{
			if (af->bitvector == AFF_HIDE)
        		mod_hide = af->level;
			}
         }

        if ( mod_hide > mod_detect)
          return FALSE;

        if ( mod_hide > ( mod_detlvl + 5 ) )
          return FALSE;
     }

   if ( IS_AFFECTED2(victim, AFF_CAMOUFLAGE)
   &&   !IS_AFFECTED2(ch, AFF_FOREST_VISION) )
        return FALSE;

   if (IS_AFFECTED2(victim, AFF_CAMOUFLAGE)
   &&  IS_AFFECTED2(ch, AFF_FOREST_VISION))
   {
     int mod_forest = MOD_NONE;
     int mod_detlvl = MOD_NONE;
     int mod_camouflage   = MOD_NONE;
     AFFECT_DATA *af;
     bool af_found;
     af_found = FALSE;

     if (IS_AFFECTED2(ch,AFF_FOREST_VISION))
	 af_found = TRUE;
     {
	 if (af_found)
       {
	   mod_forest = ch->level;
         for (af = ch->affected; af != NULL; af = af->next)
	   {
	     if (af->bitvector == AFF_FOREST_VISION)
           mod_detlvl = af->level;
	   }
       }
       else
     ...
     {
	 if (af_found)
	   mod_camouflage   = victim->level;
       else
       for (af = victim->affected; af != NULL; af = af->next)
	 {
	   if (af->bitvector == AFF_CAMOUFLAGE)
         mod_camouflage = af->level;
	 }
     }

     if ( mod_camouflage > mod_forest)
          return FALSE;

     if ( mod_camouflage > ( mod_detlvl + 5 ) )
          return FALSE;
   }
 ...
 * set_low_eq_tmr
 *
 * Sets auto cleanup timer it object is low level equipment.
 */
void set_low_eq_tmr( OBJ_DATA *obj )
{
    /* make low level eq stuff clean itself up */
    if( obj->timer == 0                     /* not already timed */
     && obj->level <= LEVEL_AUTOCLEAN       /* low level */
     && (obj->wear_flags & ~ITEM_TAKE) )    /* wearable eq (nasty trick) */
    {
        obj->timer = number_range ( (obj->level/3 + 1) * 5,
                                    (obj->level/3 + 1) * 10 );
    }
}
/*
 * clr_low_eq_tmr
 *
 * If it's low level equipment that's set for auto cleanup, clear
 * the timer
 */
void clr_low_eq_tmr( OBJ_DATA *obj )
{
    if( obj->timer != 0                     /* a timer is running */
     && obj->level <= LEVEL_AUTOCLEAN       /* low level */
     && (obj->wear_flags & ~ITEM_TAKE ) )   /* wearable (nasty trick) */
    {
        obj->timer = 0;
    }
}
...
char *affect_loc_name( int location )
{
    ...
    case APPLY_SAVING_PARA:	return "save vs pralysis";
    case APPLY_SAVING_PETRI:	return "save vs petrification";
    case APPLY_SAVING_BREATH:	return "save vs breath";
    case APPLY_SAVING_SPELL:	return "save vs spell";
    case APPLY_SPELL_AFFECT:	return "none";
    case APPLY_PERCEPTION:    return "perception";
    case APPLY_SPEECH_IMPAIR:   return "speech";
    }
...
char *affect_bit_name( int vector )
{
    ...
    if ( vector & AFF_INCENDIARY_CLOUD   ) strcat( buf, " incendiary_cloud"   );
...
char *affect2_bit_name( int vector )
{
    ...
    buf[0] = '\0';
    if ( vector & AFF_FOREST_VISION 	) strcat( buf, " forest_vision" 		);
    if ( vector & AFF_BLACKJACK     	) strcat( buf, " blackjack"     		);
    if ( vector & AFF_GLIDE        		) strcat( buf, " glide"         		);
    if ( vector & AFF_SILENCE       	) strcat( buf, " silence"	  		);
    if ( vector & AFF_FEAR       		) strcat( buf, " fear"		  		);
    if ( vector & AFF_PARALYSIS     	) strcat( buf, " paralysis"	  		);
    if ( vector & AFF_UNDETECTABLE_ALIGNMENT ) strcat( buf, " undetectable_alignment" );
    if ( vector & AFF_REPULSION		) strcat( buf, " repulsion"	  		);
    if ( vector & AFF_SYMPATHY		) strcat( buf, " sympathy"	  		);
    if ( vector & AFF_RAINBOW_PATTERN	) strcat( buf, " rainbow_pattern" 	);
    if ( vector & AFF_WRAITHFORM		) strcat( buf, " wraithform"	  		);
    if ( vector & AFF_NATURE			) strcat( buf, " natures_embrace"	  	);
    if ( vector & AFF_WARMTH			) strcat( buf, " natures_warmth"	  	);
    if ( vector & AFF_HERBS			) strcat( buf, " herbs"	  			);
    if ( vector & AFF_ENLIGHTENMENT		) strcat( buf, " enlightenment"		);
    if ( vector & AFF_DARK_TAINT		) strcat( buf, " dark_taint"		      );
    if ( vector & AFF_FORGET			) strcat( buf, " forget"			);
    if ( vector & AFF_PARADISE		) strcat( buf, " altered_reality"		);
    if ( vector & AFF_INVUL			) strcat( buf, " invulnerable"		);
    if ( vector & AFF_CAMOUFLAGE    	) strcat( buf, " camouflage"    		);
    if ( vector & AFF_LOOKING_GLASS 	) strcat( buf, " looking_glass"    		);
    if ( vector & AFF_PASSTRACE		) strcat( buf, " pass_without_trace"	);
    if ( vector & AFF_SLEEPCURSE		) strcat( buf, " sleepless_curse"		);
    if ( vector & AFF_MOONCLOAK		) strcat( buf, " mooncloak"			);
    if ( vector & AFF_BRAVADO			) strcat( buf, " bravado"			);
    if ( vector & AFF_WASTING			) strcat( buf, " wasting"			);
    if ( vector & AFF_FARSIGHT		) strcat( buf, " farsight"			);
    if ( vector & AFF_SPEECH_IMPAIR		) strcat( buf, " impaired_speech"		);
    if ( vector & AFF_DEAFNESS		) strcat( buf, " deafness"			);
    if ( vector & AFF_SWIM			) strcat( buf, " swimming"			);
    
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}

/*
 * Return ascii name of an affect3 bit vector.
 */                 
char *affect3_bit_name( int vector )
{
    ...
    buf[0] = '\0';
    if ( vector & AFF_RALLY 			) strcat( buf, " rally" 			);
    if ( vector & AFF_GAURA 			) strcat( buf, " good_aura"			);
    if ( vector & AFF_EAURA 			) strcat( buf, " evil_aura" 			);
    if ( vector & AFF_HOLDPERSON 		) strcat( buf, " holdperson" 			);
    if ( vector & AFF_IMMOLATION 		) strcat( buf, " immolation" 			);
    if ( vector & AFF_PINENEEDLES 		) strcat( buf, " pine_needles" 		);
    if ( vector & AFF_FIRE	 		) strcat( buf, " your_on_fire" 		);
    if ( vector & AFF_INSTRUCTION		) strcat( buf, " mental_fatigue"		);
    if ( vector & AFF_SUBDUE			) strcat( buf, " subdued"			);
    if ( vector & AFF_MIGHT			) strcat( buf, " might"				);
    if ( vector & AFF_VALOR			) strcat( buf, " valor"				);
    if ( vector & AFF_HONOR			) strcat( buf, " honor"				);
    if ( vector & AFF_PREACH			) strcat( buf, " preach"			);
    if ( vector & AFF_CLEARMIND		) strcat( buf, " clear_thoughts"		);
    if ( vector & AFF_DIVINE_FAVOR		) strcat( buf, " divine_favor"		);
    if ( vector & AFF_QUICKSAND		) strcat( buf, " quicksand"			);
    if ( vector & AFF_BEASTSPITE_BEAR	) strcat( buf, " beastspite_bear"		);
    if ( vector & AFF_BEASTSPITE_LIZARD	) strcat( buf, " beastspite_lizard"		);
    if ( vector & AFF_BEASTSPITE_FOX	) strcat( buf, " beastspite_fox"		);
    if ( vector & AFF_BEASTSPITE_WOLF	) strcat( buf, " beastspite_wolf"		);
    if ( vector & AFF_BEASTSPITE_WYVERN	) strcat( buf, " beastspite_wyvern"		);
    if ( vector & AFF_BEASTSPITE_DRAGON	) strcat( buf, " beastspite_dragon"		);
    if ( vector & AFF_BEASTSPITE_BAT	) strcat( buf, " beastspite_bat"		);
    if ( vector & AFF_WATCHFUL		) strcat( buf, " blessed_watchfulness"	);
    if ( vector & AFF_BLOOD_BOND		) strcat( buf, " blood_bond"			);
...
char *extra_bit_name( int extra_flags )
{
    ...
    if ( extra_flags & ITEM_MASTERWORK 	) strcat( buf, " masterwork"       	);
    ...
    if ( extra_flags & ITEM_DEATHBLADE	) strcat( buf, " deathblade"		);
    if ( extra_flags & ITEM_QUESTOBJ	) strcat( buf, " questobj"		);
    if ( extra_flags & ITEM_STAINLESS	) strcat( buf, " stainless"		);
...
char *extra2_bit_name( int extra2_flags )
{
    ...
    if ( extra2_flags & ITEM_ARTISTRY	) strcat( buf, " artistry"		);
...
char *act_bit_name( int act_flags )
{
	...
	if (act_flags & ACT_TRACK		) strcat(buf, " track");
	if (act_flags & ACT_NO_XP_GAIN	) strcat(buf, " no_xp_gain");
    ...
    {
	strcat(buf," player");
	if (act_flags & PLR_NONOTE		) strcat(buf, " no_note");
	...
	if (act_flags & PLR_CANLOOT		) strcat(buf, " loot_corpse");
      if (act_flags & PLR_AUTOTITLE   	) strcat(buf, " autotitle");
	if (act_flags & PLR_NOFOLLOW		) strcat(buf, " no_follow");
	if (act_flags & PLR_NOEXP		) strcat(buf, " no_exp");
	...
	if (act_flags & PLR_NOTITLE		) strcat(buf, " no_title");
      if (act_flags & PLR_ASSAULT		) strcat(buf, " assault");
      if (act_flags & PLR_TREASON		) strcat(buf, " treason");
      if (act_flags & PLR_OUTLAW		) strcat(buf, " outlaw");
      if (act_flags & PLR_AUTOMERCY   	) strcat(buf, " automercy");
      if (act_flags & PLR_AUTOYIELD   	) strcat(buf, " autoyield");
    }
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}

/* return ascii name of an act2 vector */
char *act2_bit_name( int act2_flags )
{
    static char buf[512];
	...
	strcat(buf," player");
      if (act2_flags & PLR_AUTODIG   	) strcat(buf, " autodig");
...
char *comm_bit_name(int comm_flags)
{
    ...
    if (comm_flags & COMM_NOPKILL		) strcat(buf, " no_pkill"	);
    ...
    if (comm_flags & COMM_NOIDLE		) strcat(buf, " no_idle"	);
...
char *wear_bit_name(int wear_flags)
{
    ...
    if (wear_flags & ITEM_WEAR_MARK		) strcat(buf, " mark");
    if (wear_flags & ITEM_SIZE_TINY		) strcat(buf, " tiny");
    if (wear_flags & ITEM_SIZE_SMALL	) strcat(buf, " small");
    if (wear_flags & ITEM_SIZE_MEDIUM	) strcat(buf, " medium");
    if (wear_flags & ITEM_SIZE_LARGE	) strcat(buf, " large");
    if (wear_flags & ITEM_SIZE_GIANT	) strcat(buf, " giant");
    if (wear_flags & ITEM_SIZE_HUGE		) strcat(buf, " huge");
    if (wear_flags & ITEM_SIZE_IGNORE	) strcat(buf, " sizeignore");
...
char *weapon_bit_name(int weapon_flags)
{
    ...
    if (weapon_flags & WEAPON_BRILLIANT 		) strcat(buf, " brilliant energy");
    if (weapon_flags & WEAPON_CHAOTIC 		) strcat(buf, " chaotic");
    if (weapon_flags & WEAPON_DISRUPTION 		) strcat(buf, " disruption");
    if (weapon_flags & WEAPON_FLAMING_BURST 	) strcat(buf, " flaming-burst");
    if (weapon_flags & WEAPON_SHOCKING_BURST 	) strcat(buf, " shocking-burst");
    if (weapon_flags & WEAPON_ICY_BURST 		) strcat(buf, " icy-burst");
    if (weapon_flags & WEAPON_HOLY 			) strcat(buf, " holy");
    if (weapon_flags & WEAPON_UNHOLY 		) strcat(buf, " unholy");
    if (weapon_flags & WEAPON_LAWFUL 		) strcat(buf, " lawful");
    if (weapon_flags & WEAPON_MIGHTY_CLEAVING 	) strcat(buf, " mighty cleaving");
    if (weapon_flags & WEAPON_THUNDERING 		) strcat(buf, " thundering");
    if (weapon_flags & WEAPON_BSTAFF 		) strcat(buf, " battle staff");
    if (weapon_flags & WEAPON_DSTAFF 		) strcat(buf, " druid staff");
...
int size_value(char *size)
{
    if (!str_cmp(size, "tiny"))
	return SIZE_TINY;
    else if (!str_cmp(size, "small"))
	return SIZE_SMALL;
    else if (!str_cmp(size, "medium"))
	return SIZE_MEDIUM;
    else if (!str_cmp(size, "large"))
	return SIZE_LARGE;
    else if (!str_cmp(size, "giant"))
	return SIZE_GIANT;
    else if (!str_cmp(size, "huge"))
	return SIZE_HUGE;
    else if (!str_cmp(size, "ignore"))
	return SIZE_IGNORE;
    else
 	return -1;
}
...
void spell_yell	(CHAR_DATA *ch, CHAR_DATA *vch, char *yell)
{
  char buf[MAX_STRING_LENGTH];
  ...
  if (!IS_AWAKE(vch))
  {
    send_to_char("You feel the need to scream!\n\r",vch);
    act("$n murmurs in $s sleep.\n\r",vch,NULL,NULL,TO_ROOM);
    return;
  }
  ...
  {
    send_to_char("You feel the need to scream!\n\r",vch);
    act("$n opens $s mouth wide, but nothing comes out.\n\r",vch,NULL,NULL,TO_ROOM);
    return;
  }

  if  (!IS_NPC(ch) && (ch->pcdata->condition[COND_DRUNK] > 20))
  {
    send_to_char("You scream out gibberish!\n\r", vch);
    act("$n screams out complete gibberish!.\n\r", vch, NULL, NULL,TO_ROOM);
    return;
  }
		
  if ((!can_see(vch,ch)) && (!is_same_group(vch,ch))) 
    do_yell(vch, "Help! Someone cast a spell on me!");
  else 
  {
    sprintf(buf, "Aaaaagh! %s cast a spell on me!",
    (is_affected(ch,gsn_doppelganger)&&!IS_IMMORTAL(vch))? ch->doppel->name :
    (is_affected(ch,gsn_treeform)&&!IS_IMMORTAL(vch))? "A mighty oak tree" :
    ch->name);
    do_yell( vch, buf);
  }
  return;
...
void check_subdue( CHAR_DATA *ch)
{
  if (IS_NPC(ch))	
    return;
  ...
  {
    REMOVE_BIT(ch->affected3_by, AFF_SUBDUE);
