...
#define CHK_AFFECTED2   (26)
#define CHK_AFFECTED3   (27)
#define CHK_ACT         (28)
#define CHK_OFF         (29)
#define CHK_IMM         (30)
#define CHK_CARRIES     (31)
#define CHK_WEARS       (32)
#define CHK_HAS         (33)
#define CHK_USES        (34)
#define CHK_NAME        (35)
...
#define CHK_ALIGN       (46)
#define CHK_MONEY       (47)
#define CHK_OBJVAL0     (48)
#define CHK_OBJVAL1     (49)
#define CHK_OBJVAL2     (50)
#define CHK_OBJVAL3     (51)
#define CHK_OBJVAL4     (52)
#define CHK_GRPSIZE     (53)
#define CHK_MANACNT     (54)
#define CHK_MOVECNT     (55)
...
const char * fn_keyword[] =
{
    ...
    "affected2",		/* if affected2 $n X	- is $n affected2 by an affect2 */
    "affected3",		/* if affected3 $n X	- is $n affected2 by an affect3 */
    ...
    "org",		      /* if org $n 'whatever'- does $n belong to clan 'whatever' */
    ...
    "manacnt",		/* if manacnt $i > 30 	- mana point percent check */
    "movecnt",		/* if movecnt $i > 30 	- move point percent check */
    "\n"			/* Table terminator */
};
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	case CHK_AFFECTED3:
	    return( lval_char != NULL 
		&&  IS_SET(lval_char->affected3_by, flag_lookup(buf, affect3_flags)) );
	case CHK_ACT:
	    return( lval_char != NULL 
	...
	case CHK_ORG:
	    return( lval_char != NULL && ORG(lval_char->org_id) == org_lookup( buf ) );
	case CHK_RACE:
	...
	case CHK_MANACNT: // Kracus- check for mana
	    if ( lval_char != NULL ) lval = (lval_char->mana * 100)/(UMAX(1,lval_char->max_mana)); break;
	case CHK_MOVECNT: // Kracus- check for movement
	...
	case CHK_MONEY:  /* Money is converted to gold... */
	    if ( lval_char != NULL ) 
		lval = lval_char->steel + (lval_char->gold * 100); break;
	case CHK_OBJVAL0:
...

{
    ...
    if ( mob )
    {
      prog = get_prog_by_vnum(pvnum, 1);
	mvnum = mob->pIndexData->vnum;
    }
    else if ( obj )
    {
      prog = get_prog_by_vnum(pvnum, 2);
	ovnum = obj->pIndexData->vnum;
    }
    else if ( room )
    {
      prog = get_prog_by_vnum(pvnum, 3);
	rvnum = room->vnum;
    }
    else
    {
	...
	*b = *c = *d = '\0';
	...
	if ( !str_cmp( control, "pause" ) )
	{
	    ...
	    {
		if(mob)
		    	sprintf( buf, "Mobprog: Pause: Argument is not a number. Using 4 as default. mob %d prog %d",
			mvnum, pvnum );
		else if(obj)
			sprintf( buf, "Objprog: Pause: Argument is not a number. Using 4 as default. obj %d prog %d",
				ovnum, pvnum );
		else
			sprintf( buf, "Roomprog: Pause: Argument is not a number. Using 4 as default. room %d prog %d",
				rvnum, pvnum );
...
/* This says if the triggering person is a PC, don't trigger.
 * If you comment this out, SPEECH triggers work fine;
    // bugfix for note and act_new if Ivan's OLC
    if(!IS_NPC(ch))
      return;
*/
...
void p_greet_trigger( CHAR_DATA *ch, int type )
{
	...
	for ( obj = ch->in_room->contents; obj != NULL; obj = obj->next_content )
	{
	    if ( HAS_TRIGGER_OBJ( obj, TRIG_GREET ) )
	    {
		p_percent_trigger( NULL, obj, NULL, ch, NULL, NULL, TRIG_GREET );
		return;
	    }
	}
	...
	if ( HAS_TRIGGER_ROOM( room, TRIG_GREET ) )
		p_percent_trigger( NULL, NULL, room, ch, NULL, NULL, TRIG_GREET );
    }
...
void p_hprct_trigger( CHAR_DATA *mob, CHAR_DATA *ch )
{
 ...
 PROG_CODE *get_prog_by_vnum(int vnum, int type )
 {
     PROG_CODE *temp;
   ...
   {
     for(temp = oprog_list; temp != NULL; temp = temp->next)
         if(temp->vnum == vnum)
             return temp;
    return NULL;
   }
   ...
   {
     for(temp = rprog_list; temp != NULL; temp = temp->next)
         if(temp->vnum == vnum)
             return temp;
    return NULL;
   }
