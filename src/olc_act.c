...
const struct olc_help_type help_table[] =
{
    ...
    {	"control",		control_flags,	"Control flags for area."	},
...
bool show_help( CHAR_DATA *ch, char *argument )
{
          ...
          && !str_prefix( arg, help_table[cnt].command ) )
	{
	    ...
	    if ( help_table[cnt].structure == trap_table )
	    {
	        show_traplist( ch );
	        return FALSE;
	    }
		...
		if ( spell[0] == '\0' )
		{
		    send_to_char( "Syntax:  ? spells "
		        "[ignore/attack/aggravate/defend/self/all]\n\r", ch );
		    return FALSE;
		}
		...
		if ( !str_prefix( spell, "all" ) )
		...
		else if ( !str_prefix( spell, "aggravate" ) )
		    show_skill_cmds( ch, TAR_CHAR_SEMIOFFENSIVE );
		    ...
		    send_to_char(
		        "You can not use object targeted spells, they crash the MUD!\n\r", ch );
		else
...
/*    buf1[0] = '\0'; */
	...
	if ( ( pRoomIndex = get_room_index( vnum ) ) )
	{
		found = TRUE;
                //C045
                sprintf( buf, "[%5d][%s] %-15.13s{x",
                         vnum,
                         (pRoomIndex->description!=&str_empty[0]) ? "{GY{x" : "{RN{x",
                         capitalize( strip_color(pRoomIndex->name) ) ); //added strip_color so that the table formatting dosent get messed up
		add_buf( buf1, buf );
		if ( ++col % 3 == 0 )
		    add_buf( buf1, "\n\r" );
	}
...
REDIT( redit_oshow )
{
    ...
    if ( !is_number( argument ) )
    {
       send_to_char( "REdit: The vnum must be a number.\n\r", ch);
       send_to_char( "Syntax:  oshow <vnum>\n\r", ch );
       return FALSE;
    }
...
AEDIT( aedit_show )
{
    ...
    sprintf( buf, "Continent:     [%s] Do not set if you do not know what your doing!\n\r", pArea->continent);
    send_to_char( buf, ch );

    sprintf( buf, "Controlled By: [%s]  [%d]\n\r", flag_string(control_flags, pArea->control_flags), pArea->faction );
    send_to_char( buf, ch );
    ...
    sprintf( buf, "Sector:        [%s]\n\r", flag_string(sector_flags, pArea->sector_type) );
    send_to_char( buf, ch );
...
AEDIT( aedit_continent )
{
    ...
    if ( argument[0] == '\0' )
    {
	send_to_char( "Syntax:   continent [$continent]\n\r", ch );
	return FALSE;
    }

    free_string( pArea->continent );
    pArea->continent = str_dup( argument );     
    send_to_char( "Continent name set.\n\r", ch );
    return TRUE;
}
...
AEDIT( aedit_control )
{
	...
	int value;
      ...
      if(ch->level < 110)
	{
	  send_to_char("Only Implementors may SET this variable. Please inform them if you wish to have this Flag altered.\n\r", ch);
        return FALSE;
	}

	if( (value = flag_value(control_flags, argument)) == NO_FLAG )
	{
	  send_to_char( "Syntax:   Control <flag>   Use '? control' to view control flags.\n\r",ch);
	  return FALSE;
	}

	pArea->control_flags = value;
	send_to_char("Control flag set.\n\r", ch);

	if( (value = flag_value(control_flags, argument)) == CONTROL_SOLAMNICS )
	{
	  pArea->faction = 250;
	}
      else
	if( (value = flag_value(control_flags, argument)) == CONTROL_KOT )
	{
	  pArea->faction = -250;
	}
      else
...
AEDIT( aedit_faction )
{
    ...
    if(ch->level < 110)
    {
	send_to_char("Only Implementors may SET this variable. Please inform them if you wish to have this value altered.\n\r", ch);
      return FALSE;
    }

    if ( !is_number( buf ) || buf[0] == '\0' )
    {
	send_to_char( "Syntax:  faction [#range]\n\r", ch );
	return FALSE;
    }
    ...
    {
	send_to_char( "Faction range is -250 to 250.\n\r", ch );
	return FALSE;
    }

    pArea->faction = value;

    send_to_char( "Faction value set.\n\r", ch );
    return TRUE;
}
...
AEDIT( aedit_sector )
{
	...
	{
		send_to_char( "Syntax:   sector [$sector]   Use '? sector' to view sector types.\n\r",ch);
		return FALSE;
	}
...
/* ARMOR for ROM */
		...
		obj->value[2],
		obj->value[3],
		flag_string (armor_flags, obj->value[4]));
	    send_to_char( buf, ch );
	    break;
	  ...
	  case ITEM_ARROW:
	    ...
	    switch ( value_num )
	    {
	        ...
	        case 2:
	            ...
	            break;
	        case 3:
			if ((value = skill_lookup( argument )) == skill_lookup("summon"))
        	      {
            	    send_to_char("Nope! You can NOT add that spell to an object!\n\r", ch );
            	    return FALSE;
        		}
			if ((value = skill_lookup( argument )) == skill_lookup("gate"))
        	      {
            	    send_to_char("Nope! You can NOT add that spell to an object!\n\r", ch );
            	    return FALSE;
        		}
			if ((value = skill_lookup( argument )) == skill_lookup("dimensional door"))
        	      {
            	    send_to_char("Nope! You can NOT add that spell to an object!\n\r", ch );
            	    return FALSE;
        		}
			if ((value = skill_lookup( argument )) == skill_lookup("earthmeld"))
        	      {
            	    send_to_char("Nope! You can NOT add that spell to an object!\n\r", ch );
            	    return FALSE;
        		}
			if ((value = skill_lookup( argument )) == skill_lookup("shadow walk"))
        	      {
            	    send_to_char("Nope! You can NOT add that spell to an object!\n\r", ch );
            	    return FALSE;
        		}
			if ((value = skill_lookup( argument )) == skill_lookup("nexus"))
        	      {
            	    send_to_char("Nope! You can NOT add that spell to an object!\n\r", ch );
            	    return FALSE;
        		}
			if ((value = skill_lookup( argument )) == skill_lookup("banish"))
        	      {
            	    send_to_char("Nope! You can NOT add that spell to an object!\n\r", ch );
            	    return FALSE;
        		}
			if ((value = skill_lookup( argument )) == skill_lookup("cantrip"))
        	      {
            	    send_to_char("Nope! You can NOT add that spell to an object!\n\r", ch );
            	    return FALSE;
        		}
			if ((value = skill_lookup( argument )) == skill_lookup("word of death"))
        	      {
            	    send_to_char("Nope! You can NOT add that spell to an object!\n\r", ch );
            	    return FALSE;
        		}
...
/* ARMOR for ROM: */
	    ...
	    switch ( value_num )
	    {
		    ...
		    break;
		  case 4:
	          if ((value = flag_value(armor_flags, argument)) != NO_FLAG )
...
/* WEAPONS changed in ROM */
	    ...
	    switch ( value_num )
	    {
	        ...
	        case 3:
	            ...
	            break;
	        case 4:
			if (((value = flag_value(weapon_type2, argument)) == WEAPON_BSTAFF )
			&& (ch->level != 110))
        	      {
            	    send_to_char("Nope! Only Implementors may use this Special Flag!\n\r", ch );
            	    return FALSE;
        		}
			if (((value = flag_value(weapon_type2, argument)) == WEAPON_DSTAFF )
			&& (ch->level != 110))
        	      {
            	    send_to_char("Nope! Only Implementors may use this Special Flag!\n\r", ch );
            	    return FALSE;
	...
	case ITEM_FURNITURE:
	    switch ( value_num )
	    {
	        ...
	        case 3:
                  if ( atoi(argument) > 10 )
        	      {
            	    send_to_char("Healing bonus can NOT be set over 10.\n\r", ch );
            	    return FALSE;
        		}
	        ...
	        case 4:
                  if ( atoi(argument) > 10 )
        	      {
            	    send_to_char("Mana bonus can NOT be set over 10.\n\r", ch );
            	    return FALSE;
        		}
		...
		case 4:
        	    ...
        	    {
            	send_to_char("Weight Multiplier can NOT be set over 100.\n\r", ch );
            	return FALSE;
        	    }
                if ( atoi(argument) < 80 )
        	    {
            	send_to_char("Weight Multiplier can NOT be less than 80.\n\r", ch );
            	return FALSE;
        	    }
...
/* OLC		    do_help( ch, "liquids" );    */
	...
	case ITEM_FOOD:
	    switch ( value_num )
	    {
	        ...
	        case 0:
                  if ( atoi(argument) < 0 )
        	      {
            	  send_to_char("WRONG! You can NOT set Food hours below 0.\n\r", ch );
            	  return FALSE;
        		}
	...
	case ITEM_BOOBYTRAP:
	    ...
	    {
	        default:
		    do_help( ch, "ITEM_TRAPS" );
	            return FALSE;
	        case 1:
			if (((value = trap_lookup(argument)) == 2 )
			&& (ch->level != 110))
        	      {
            	  send_to_char("Nope! Only Implementors may use that Trap Flag!\n\r", ch );
            	  return FALSE;
        		} 
	            send_to_char( "TRAP TYPE SET.\n\r\n\r", ch );
	            pObj->value[1] = ( trap_lookup(argument) != -1 ?
	            		       trap_lookup(argument) : 0 );
	            break;
	    }
            break;
...
OEDIT( oedit_create )
{
    ...
    if ((( atoi(argument)  >= 1230)
    &&   ( atoi(argument)  <= 1299))
    &&   (ch->level != 110))
    {
	send_to_char( "OEdit: You are NOT authorized to create an Object using that VNUM.\n\r",ch );
	return FALSE;
    }
...
OEDIT( oedit_material )
{
    ...
    if ( argument[0] != '\0' 
    && (material = material_lookup(argument)) != -1 )
    {
	EDIT_OBJ(ch, pObj);

   	free_string( pObj->material );
	pObj->material = str_dup(material_list[material].name);
	...
	buffer = new_buf();
	add_buf(buffer, "Available Materials are:");

	for ( material = 0; material_list[material].name != NULL; material++ )
	{
		if ( (material % 4) == 0 )
		   add_buf(buffer, "\n\r");
		sprintf( buf, "%-20s", material_list[material].name );
		add_buf(buffer, buf);
	}
	add_buf(buffer, "\n\r");
    ...
    send_to_char( "Syntax:  material [material]\n\r"
...
MEDIT( medit_spec )
{
    ...
    if ((spec_lookup(argument) == spec_lookup("SPEC_FERRYSOL") )
    && (ch->level != 110))
    {
    send_to_char("Nope! Only Implementors may use this Special Flag!\n\r", ch );
    return FALSE;
    ...
    if ((spec_lookup(argument) == spec_lookup("SPEC_FERRYPAL") )
    && (ch->level != 110))
    {
    send_to_char("Nope! Only Implementors may use this Special Flag!\n\r", ch );
    return FALSE;
    ...
    if ((spec_lookup(argument) == spec_lookup("SPEC_ORGGUARD") )
    && (ch->level != 110))
    {
    send_to_char("Nope! Only Implementors may use this Special Flag!\n\r", ch );
    return FALSE;
...
MEDIT( medit_damtype )
{
	...
	send_to_char( "In order to see a list of messages, type '? weapon'.\n\r", ch );
	return FALSE;
    }
...
MEDIT( medit_align )
{
    ...
    if ( ( atoi(argument) > 1000 )
    ||   ( atoi(argument) < -1000))
    {
      send_to_char("Alignment can NOT be set higher than 1000 or lower than -1000.\n\r", ch );
      return FALSE;
    }
...
MEDIT( medit_material )
{
    ...
    /* We will phase out mob material soon anyway. */
    send_to_char("Material on mobs is un-nessasary. Do not set.\n\r",ch);
    return FALSE;
	...
	pMob->material = str_dup(material_list[material].name);
	...
	send_to_char( "Available Materials are:", ch );
...
MEDIT( medit_org )
{
	...
	if( !str_cmp(argument, "none") )
	{
		value = 0;
		pMob->org_id = value;
		send_to_char("Org cleared.\n\r", ch);
		return TRUE;
	}
	else if( (value = org_lookup(argument)) > 0 )
	{
		pMob->org_id = value;
		send_to_char("Org set.\n\r",ch);
		return TRUE;
	}
      else
      send_to_char( "That is an Invalid Organization!\n\r", ch );
	return FALSE;
    }

    send_to_char( "Syntax: org [org] - Available organizations:\n\r",ch);
    for(org = 0; org < MAX_ORG; org++)
    {
	printf_to_char(ch, "%-13s", org_table[org].name);
	if( org > 0 && (org % 4) == 0 ) send_to_char("\n\r",ch);
    }
    send_to_char("\n\r",ch);
    return FALSE;
}
...
void show_traplist(CHAR_DATA *ch)
{
    int trap;
    BUFFER *buffer;
    char buf[MAX_STRING_LENGTH];
    ...
    for ( trap = 0; trap_table[trap].trap_name != NULL; trap++)
    {
	if ( (trap % 21) == 0 )
	    add_buf(buffer," Trap Name\n\r");

	sprintf(buf, "%-20s\n\r",
		trap_table[trap].trap_name );
	add_buf(buffer,buf);
    }
...
MEDIT( medit_autoset )
{
    ...
    ac_n = 95 - (pMob->level * 4.67) - ((pMob->level/10)^2);
    ac_x = 95 - (pMob->level * 2.57) - ((pMob->level/10)^2);
    ...
    if (size < 2)
        size = 2;
    bonus = (5.5 + pMob->level/3);
    sprintf(temp, "%dd%d + %d", dice, size, bonus);
    medit_damdice(ch, temp);

    bonus = (pMob->level/55 + 1) * ((pMob->level * 9) + (pMob->level/10)) +1; 
    size = pMob->level/3;
    if (size < 2)
        size = 2;
    dice = (pMob->level + .5) * 1.5 + .2;
    if (dice < 1)
        dice = 1;
    sprintf(temp, "%dD%d + %d", dice, size, bonus);
...
REDIT( redit_room )
{
	...
	if ( (value = flag_value( room_flags, argument )) == NO_FLAG )
	{
        ...
        if (( (value = flag_value( room_flags, argument )) == ROOM_LIGHT )
        &&  IS_SET(room->room_flags, ROOM_DARK) )
        {
	  send_to_char("That's not possible when the room is already set to DARK.\n\r",ch);
          send_to_char("Remove the DARK flag first, then try again!\n\r",ch);
	  return FALSE;
        }

        if (( (value = flag_value( room_flags, argument )) == ROOM_DARK )
        &&  IS_SET(room->room_flags, ROOM_LIGHT) )
        {
	  send_to_char("That's not possible when the room is already set to LIGHT.\n\r",ch);
          send_to_char("Remove the LIGHT flag first, then try again!\n\r",ch);
	  return FALSE;
        }
