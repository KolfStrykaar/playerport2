...
/* stuff for recyling emails */
EMAIL_DATA *email_data_free;

EMAIL_DATA *new_email_data()
{
    EMAIL_DATA *email_data;

    if (email_data_free == NULL)
	email_data = alloc_perm(sizeof(*email_data));
    else
    { 
	email_data = email_data_free;
	email_data_free = email_data_free->next;
    }
    VALIDATE(email_data);
    return email_data;
}

void free_email_data(EMAIL_DATA *email_data)
{
    if (!IS_VALID(email_data))
	return;

/* free_string won't clear it, so we do it manually. - Shaun */

    if(email_data->to != NULL)
    {
    	free_mem(email_data->to, strlen(email_data->to) +1);
    	email_data->to = NULL;
    }

    if(email_data->subject != NULL)
    {
    	free_mem(email_data->subject, strlen(email_data->subject) +1);
    	email_data->subject = NULL;
    }

    if(email_data->body != NULL)
    {
   	 free_mem(email_data->body, strlen(email_data->body) +1);
    	 email_data->body = NULL;
    }

    INVALIDATE(email_data);
    email_data->next = email_data_free;
    email_data_free   = email_data;
}
