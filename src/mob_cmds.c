...
// Kracus- Group Transfer: victin and gch has to be in same room as Mob
//           -victim(char being transferred)
//           -victim->pet(char's pet being transferred, set TRUE)
//           -gch(group members of victim)
//           -gch->pet(group member's pet NOT transferred)
...
void do_mpgtransfer( CHAR_DATA *ch, char *argument )
{
    ...
    if (victim->pet != NULL && victim->in_room == victim->pet->in_room)
        mpgtransfer_pet = FALSE; // Set to TRUE if you want pet transfer
    else
        mpgtransfer_pet = FALSE;
   
    // Save roommates of the victim for group gate
    room = victim->in_room;
    ...
    // Stop the victim if he is in a fight
    if (victim->fighting != NULL)
        stop_fighting(victim, TRUE);
    ...
    // Transfer all group member's of the victim that were in the original room
    for (gch = room->people; gch != NULL; gch = gch_next)
    {
        gch_next = gch->next_in_room;
  
        // Stop the gch if he is in a fight
        if (gch != victim && gch->fighting != NULL)
...
void do_mpremove( CHAR_DATA *ch, char *argument )
{
	...
	{
           // Kracus- check for wear_loc if not wearing unequip, extract
          if (obj->wear_loc != -1)
	    {
	        unequip_char( ch, obj );
	        obj_from_char (obj );
	    ...
	    else // Kracus- else if not wearing, but in inventory extract
	    {
	        obj_from_char( obj );
	        extract_obj( obj );
	    }
...
/*
 * Lets send the player home to their respective Recall Room
 * Useful for removing e.g. quest objects from a character.
 *
 * Syntax: mob sendhome [victim]
 */

void do_mpsendhome(CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    ROOM_INDEX_DATA *location;
    char arg[MAX_INPUT_LENGTH];
    ...
    if ( arg[0] == '\0' )
    {
	bug( "Mpsendhome - Missing argument from vnum %d.", 
		IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
	return;
    }
    ...
    if ( ( victim = get_char_world( ch, arg ) ) == NULL )
    {
	bug( "Mpsendhome - No such person in vnum %d.", 
		IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
	return;
    }
    ...
    if ( victim->desc == NULL )
    {
	bug( "Mpsendhome - No descriptor for victim in vnum %d.", 
		IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
	return;
    }
    ...
    {
	bug( "Mpsendhome - Trying to sendhome a mob from vnum %d.", 
		IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
	return;
    }

    //C053
/*    if ((location = get_room_index(clan_table[victim->clan].room[0] )) == NULL)
 if ( ( location = get_room_index( ROOM_VNUM_TEMPLE ) ) == NULL )*/
    if ((location=get_home(victim))==NULL)
    {
	bug( "Mpsendhome - No place to send them to from vnum %d.", 
		IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
	return;
    }
...
void do_opattrib( OBJ_DATA *obj, char *argument )
{
	...
	if ( !is_number( arg1 ) )
	{
	    bug( "OpAttrib: received non-number argument from vnum %d",
		 obj->pIndexData->vnum );
	    return;
	}
	...
	case '/':
	    ...
	    break;
	default:
	    bug( "OpAttrib: invalid modifier from vnum %d", obj->pIndexData->vnum );
	    return;
	}
    }
...
/*
 * Send the victim home to their respective recall room
 * Syntax: obj sendhome <victim>
 */

void do_opsendhome( OBJ_DATA *obj, char *argument )
{
    CHAR_DATA *victim;
    ROOM_INDEX_DATA *location;
    char arg[MAX_INPUT_LENGTH];
    ...
    if ( arg[0] == '\0' )
    {
	bug( "Opsendhome - Missing argument from vnum %d.", 
		obj->pIndexData->vnum );
	return;
    }
    ...
    {
	bug( "Opsendhome - No such person in vnum %d.", 
		obj->pIndexData->vnum  );
	return;
    }
    ...
    if ( victim->desc == NULL )
    {
	bug( "Opsendhome - No descriptor for victim in vnum %d.", 
		obj->pIndexData->vnum );
	return;
    }
    ...
    if (IS_NPC(victim) && !IS_SET(victim->act,ACT_PET))
    {
	bug( "Opsendhome - Trying to sendhome a mob from vnum %d.", 
		obj->pIndexData->vnum );
	return;
    }

    if(victim->in_room == get_room_index(1280) )
    {
	send_to_char("Escaping out of jail doesn't seem like the best idea....\n\r",victim);
	return;
    }
...
/*    if ((location = get_room_index(clan_table[victim->clan].room[0] )) == NULL)
 if ( ( location = get_room_index( ROOM_VNUM_TEMPLE ) ) == NULL )*/
    if ((location=get_home(victim))==NULL)
    {
	bug( "Opsendhome - No place to send them to from vnum %d.", 
		obj->pIndexData->vnum );
	return;
    }
